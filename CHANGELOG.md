...
### Aktivierungs-Kosten-Transparenz für EEG-Admins (PROJ-111)

Der EEG-Admin sieht im **„Rechnungen"-Tab** jetzt eine Karte **„Aktivierungs-Kosten · laufendes Quartal"** über der Rechnungsliste: aktuelle Edition, Stückpreis je aktiviertem Mitglied (oder „kein Tarif hinterlegt"), Aufschlüsselung Standard/Pro, rohe und **voraussichtliche** Quartals-Summe, Trial-Hinweis („kostenlos – bis &lt;Datum&gt;") sowie ein dezenter „Vorschau"-Tag, solange nicht live abgerechnet wird. Read-only Projektion — **keine** Migration, kein Vendor-Call, keine Schreibpfade.

- **Single-Source:** Zahlen kommen serverseitig aus denselben Quellen wie die echte Abrechnung. Dafür wurde `PricingService.CalculateQuarter` in nebenwirkungsfreie Helfer zerlegt: `projectPeriod` (kein UPSERT) + ein idempotenter Upsert; read-only `resolveTrialAnchor` (ohne Audit-Write, der bleibt im Cron-Pfad); gemeinsame `computeTotals`; geteilte Quartals-Ableitung `CurrentQuarter(now)`. Cron-Verhalten bit-identisch (Pinning-Tests unverändert grün).
- **Pfad-unabhängig:** Gezählt wird auf `activated_at` × `edition_at_activation` — manuelle **und** Batch-Aktivierungen (PROJ-46) sind automatisch erfasst.
- **Auslieferung:** der bestehende, tenant-geprüfte `GET /api/admin/eeg/{rc}/invoices` liefert zusätzlich ein `currentQuarter`-Objekt (best-effort, Fehler → `null`). Owner-Billing-Sicht unverändert.
...
- **Registrierungs-Toggle blieb nach „Plattform buchen" gesperrt.** Nach einer Buchung über die Settings-Karte aktualisierte sich nur die Karte selbst — die Settings-Seite las `canActivateRegistration` (AVV-Gate) nicht neu, sodass der Toggle „Mitgliederregistrierung aktiv" trotz vorliegender Buchung ausgegraut blieb (bis Reload). Fix: `onBooked`-Callback erhöht `applyEpoch` → `getEEGSettings` lädt neu. Backend-Gate war korrekt.
- **„Vertrag & Onboarding-Status"-Karte kompakter.** Die ausführliche, große Karte (redundante EEG-Name/Kontakt-Zeilen) ist jetzt eine dezente einzeilige Status-Zeile mit getöntem Chip + AGB/AVV-Links + AVV-PDF-Button, auf Semantic-Tokens (passt sich dem dunklen Admin-Theme an).
- **AVV-Akzept-PDF passt auf eine Seite.** Der Footer wurde manuell via `SetY(-15)`+`CellFormat` platziert und löste — weil das unter dem AutoPageBreak-Trigger liegt — einen Seitenumbruch auf eine zweite, sonst leere Seite aus. Auf `SetFooterFunc` umgestellt + Regressions-Test (1 Seite auch bei voller Stammdaten-Variante).
- **Plattform-Buchungen-Detailseite zeigte überall `undefined` / `Invalid Date`.** Das Owner-BackOffice-Detail gibt `Submission`/`EntrypointStammdaten`/`ContractStatus` direkt aus, aber diese Structs hatten **keine** json-Tags → Go marshalte PascalCase (`RCNumber`, `AGBVersion`, …), das Frontend liest camelCase. camelCase-Tags ergänzt + Regressions-Test; der AVV-PDF-Blob wird jetzt zusätzlich mit `json:"-"` aus der Detail-Antwort ausgeschlossen (eigener Download-Endpoint). Die Liste war nie betroffen (eigenes DTO).
- **Plattform-Willkommens-Mail: obsoleter „Login wird eingerichtet"-Schritt entfernt.** Die Mail an die EEG (nach Buchung/Aktivierung) beschrieb, dass der Plattform-Betreiber sich zur Einrichtung eines Admin-Logins meldet — das stammt aus der Public-Form-Ära. Die EEG bucht heute aus den Admin-Einstellungen heraus, ist also bereits per Keycloak eingeloggt. Schritt entfernt, Konfigurations-Hinweis auf „mit Ihrem bestehenden Login" umformuliert; dieselbe obsolete Zeile auch aus der Owner-FYI-Mail entfernt.
- **Plattform-Buchung blockiert ohne synchronisierte EEG-Stammdaten.** Buchte eine EEG, bevor ihre Stammdaten aus eegFaktura synchronisiert waren, hatte die generierte **AVV-Akzept-PDF einen leeren Auftraggeber** (kein Vereinsname/Anschrift/Kontakt) — ein ungültiger AVV. Server-Guard (`stammdatenCompleteForAVV`) in `Submit` bricht die Buchung mit klarer Meldung (`stammdaten_not_synced`) ab, bis Vereinsname, Anschrift und Kontakt-E-Mail vorliegen; die Buchungs-Karte deaktiviert den „Plattform buchen"-Button vorab mit Hinweis „erst aus eegFaktura aktualisieren". (Tester-Befund 2026-06-20.)
- **Mehrere EEG-Kontakt-Adressen werden korrekt zugestellt.** Enthält die aus eegFaktura gesyncte Kontakt-E-Mail mehrere Adressen (`;`- oder `,`-getrennt, z. B. `a@b.de;c@d.com`), wurde der ganze String als **eine** Adresse an den Mailer gegeben — go-mail lehnt das ab, die Mail wurde **nicht versendet** (bei der Hart-Fail-Beitrittsbestätigung-an-EEG sogar mit Rollback des Aktivierungs-Übergangs). Zentral im Mailer (`splitRecipients`) behoben: To adressiert jetzt **alle** Adressen, Reply-To die erste. Wirkt für **alle** Mail-Pfade. (Tester-Befund 2026-06-20.)
