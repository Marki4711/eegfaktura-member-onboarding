...
The following points from `docs/import-mapping.md §7` must be confirmed with the eegFaktura core team before implementation:

| # | Question | Impact |
|---|---|---|
| 1 | Is `participantNumber` generated by the core, or must it be supplied? | Determines whether onboarding sets it or leaves it empty |
| 2 | Can `businessRole` and `role` be empty strings or omitted? | Required for payload validation |
| 3 | Can all `accountInfo` fields be empty? | Required for payload validation |
| 4 | Are `meters[].gridOperatorName` and `meters[].gridOperatorId` required? | If required, they have no onboarding source — needs a default or a new field |
| 5 | Must `participantState` be explicitly included or does the core set it? | Determines whether the adapter sends it or omits it |
| 6 | Is `partFact = 100` the correct technical default? | Applied to every metering point in V1 |
| 7 | What HTTP method and path does the core expose for participant creation? | Needed to build the internal core client |
| 8 | What auth mechanism does the core require for internal calls (service token, mTLS, none)? | Needed for the core client; may block implementation until core is reachable |
| 9 | Is the core's participant creation idempotent? | Critical for retry safety |
| 10 | What does a successful core response body look like, and where is `targetParticipantId` returned? | Required for parsing the response |
