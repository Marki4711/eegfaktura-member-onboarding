...
| `→ activated` (PROJ-76 Vorstands-Modus `board_approval_workflow_enabled=true`) | ❌ (Member wird über reguläre eegFaktura-Core-Aktivierungs-Mail informiert) | — | ✅ **Beitrittserklärung** mit Vorstands-Signaturblock zur Unterzeichnung | **sync hard-fail** vor Commit; bei SMTP-Fehler oder fehlendem `contact_email` rollt der Status-Wechsel zurück; idempotent via `board_declaration_sent_at` |
| `approved → activated` (PROJ-53 manueller Skip-Import via `POST /mark-activated`) | gleiche Logik wie regulärer Activate je nach Modus | — | gleiche Logik | gleiche Idempotenz/Hart-Fail-Regeln je nach Modus |
