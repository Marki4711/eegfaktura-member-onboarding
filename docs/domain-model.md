...
- `sepa_mandate_enabled` — boolean, default false; controls whether SEPA mandate PDF is attached to the Core (Privat) welcome email. The per-application mandate variant (Core vs B2B) lives in `application.einzugsart` (`core` | `b2b` | `kein_sepa`) and is set by the admin per application. **Wirkt nur auf Core-Mandate.** PROJ-74 (2026-06-06): B2B-Mandate (`einzugsart=b2b`) werden in jedem Fall als PDF beim Import generiert, weil das SEPA-Regelwerk eine unterschriebene Mandatsvorlage für Firmenlastschriften verlangt; eine reine Online-Zustimmung ist hier nicht zulässig. Fehlende EEG-Stammdaten brechen den B2B-Import hart ab (Hart-Fail), für Core-Anträge bleibt es bei Skip+Warn (Online-Zustimmung als Fallback). PROJ-73 (2026-06-06) entfernte zudem den obsoleten `use_company_sepa_mandate`-Toggle, der seit PROJ-48 funktionslos war.
